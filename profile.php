...
for ($i=($num_tek-2); $i>=($num_tek-$num-2); $i--)
...
echo $j;
echo '</br>';
echo $adr_req.$cod_req.$mem_req.$num_req.$crc_req;
echo '</br>';
$active_array[$j] = $active_prof/2;
$j++;
};
...
$aSheet->setCellValue('G2','о фактическом почасовом расходе электрической энергии за '.$mounth_otch.' 2014 года ОАО "Аньковское"');
...
$aSheet->getStyle('A2')->getFont()->setBold(true);
$aSheet->setCellValue('A2','о расходе электроэнергии по ОАО "Аньковское", за '.$mounth_otch.' 2014 года.');
...
$objWriter->save('/home/public/otch/'.$mounth_otch.'-'.$year_otch.'-Профиль мощности1.xls');
